...
完整说明：[docs/PRE_COMMIT_HOOK_zh.md](docs/PRE_COMMIT_HOOK_zh.md) · [English](docs/PRE_COMMIT_HOOK.md)
...
- **API 文档：** [docs/API_zh.md](docs/API_zh.md)（[English](docs/API.md)）
...
| `screen/decks/` | 牌组列表 Tab：`decks_screen.dart`、`providers/`（列表、创建牌组）、`widgets/`。 |
| `screen/deck/` | 学习与牌组编辑：`deck_learn_screen.dart`、`deck_detail_screen.dart`、`providers/`（卡片状态、复习间隔、音频）、`widgets/`（卡片、闪卡、媒体、知识点）。 |
| `screen/profile/` | 个人资料 Tab 及相关 providers。 |

功能的 provider 通常与页面同级（`screen/<功能>/providers/`）。部分页面使用嵌套的 **`ProviderScope`** 与 **overrides**（例如 `deck_learn_screen.dart` 向 `cardProvider` 注入当前 `Deck`）。

### 共享 UI 与工具

- **`widgets/`** — 可复用组件（`common_refresher`、`common_bottom_sheet`、`video_player/` 子树等）。
- **`utils/`** — 日志、防抖、小工具。
...
测试位于 `test/`，结构尽量与 `lib/` 对应（`test/providers/`、`test/screen/`、`test/services/`、`test/models/` 及 `test/helpers/` 等）。命令与约定见 [docs/FRONTEND_TESTS_zh.md](docs/FRONTEND_TESTS_zh.md)（[English](docs/FRONTEND_TESTS.md)）。
